...
exports.stripeCheckout2 = functions.https.onCall(async (data, context) => {
    var productname = data['productname'];
    var amount1 = data['amount'];
    var companyname = data['compname'];
    var quantity = data['quantity'];
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        payment_intent_data:{receipt_email:'oscarchuaweiwen.personal@gmail.com'},
        mode: 'payment',
        success_url: 'http://localhost:4200/studentpage?action=success'+ `&productname=${productname}&amount=${amount1}&studentname=${studentname}&quantity=${quantity}&category=${category}&trackingnumber=${trackingnumber}`,
        cancel_url: 'http://localhost:4200/studentpage?action=cancel'+ `&productname=${productname}&amount=${amount1}&studentname=${studentname}&quantity=${quantity}&category=${category}&trackingnumber=${trackingnumber}`
    });

    return {session:session.id,ingredient:companyname};
})
