...
export class AdminCartPageComponent implements OnInit {
  ...
  getInfo(e: any, row1: any) {
    if (this.status) {
      this.overallquantity = e.value;
      console.log(this.products);
    }

    if (this.status) {
      console.log(this.products[row1]);
    }
  }
  ...
  deleteCart(brand:any){
    ...
    this.fs.collection('Admin') .doc('oMWhzMQgufX3WpRQs9WsB4JmQFv2').collection('cart').doc(brand).delete();
  }
}
